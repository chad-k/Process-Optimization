...
def compute_results_dynamic(model_df, param_cols, bounds_by_param, bounds_overrides, soft_tol_percent, model_choices, min_rows_per_group):
    ...
    for (part, machine), group in model_df.groupby(["part_key", "machine_key"]):
        # Allow per-group overrides (keyed by (part_key, machine_key))
        group_key = (str(part), str(machine))
        group_over = bounds_overrides.get(group_key, {}) if isinstance(bounds_overrides, dict) else {}
...
with st.sidebar:
    ...
    auto_run = st.checkbox("Auto-run optimization on changes", value=True)
    soft_tol_percent_ui = st.slider("Soft tolerance (±%) around target", 0.0, 50.0, 10.0, 0.5)
    min_rows_per_group = st.number_input("Min rows per Part+Machine group", min_value=5, max_value=500, value=10, step=1)
...
with c1:
    ...
    part_col = st.selectbox("Part column", options=data_cols, index=pick_default(data_cols, tpl_dm.get("part_col") or guess_part))
    machine_col = st.selectbox("Machine column", options=data_cols, index=pick_default(data_cols, tpl_dm.get("machine_col") or guess_machine))

    measurement_mode = st.selectbox("Measurement layout", options=MEAS_MODES,
                                    index=pick_default(MEAS_MODES, tpl_dm.get("measurement_mode") or "Auto (Mixed)"))
    agg_func = st.selectbox("Aggregation (for subgroup)", options=["mean", "median"],
    ...
    wide_cols = st.multiselect("Wide subgroup member columns (Format C) (optional for Auto)",
                               options=data_cols, default=default_wide[:30])

with c2:
    st.markdown("### Specs mapping")
    specs_part_col = st.selectbox("Specs Part column", options=specs_cols,
                                 index=pick_default(specs_cols, tpl_sm.get("specs_part_col") or guess_specs_part))
    target_col = st.selectbox("Target column", options=specs_cols,
                              index=pick_default(specs_cols, tpl_sm.get("target_col") or guess_target))

    lsl_opts = ["(none)"] + specs_cols
    usl_opts = ["(none)"] + specs_cols

    lsl_pick = st.selectbox("LSL column (optional)", lsl_opts, index=pick_default(lsl_opts, tpl_sm.get("lsl_col") or guess_lsl or "(none)"))
    usl_pick = st.selectbox("USL column (optional)", usl_opts, index=pick_default(usl_opts, tpl_sm.get("usl_col") or guess_usl or "(none)"))
...
param_cols = st.multiselect("Select parameter columns to optimize", options=param_candidates, default=default_params)
...
# ------------------------------
# Step 3 — Bounds (defaults + per Part–Machine overrides)
# ------------------------------
st.subheader("Step 3 — Bounds (auto-suggested defaults, with per Part–Machine overrides)")

# Defaults are suggested from RAW data and apply to all groups unless overridden.
# Overrides are keyed by (part_key, machine_key) and do NOT affect other groups.

with st.sidebar:
    st.header("Bounds suggestion (defaults)")
    auto_bounds = st.checkbox("Auto-suggest default bounds", value=True)
    ...
    per_group_suggest = st.checkbox("Suggest defaults from one Part+Machine subset", value=False)
...
# Session state for bounds
if "bounds_default" not in st.session_state:
    st.session_state.bounds_default = {}
if "bounds_overrides" not in st.session_state:
    # { (part_key, machine_key): {param: (lo, hi), ...}, ... }
    st.session_state.bounds_overrides = {}

# Build/update default bounds
reset_defaults = st.sidebar.button("Reset ALL default bounds to suggested", use_container_width=True)
...
for pcol in param_cols:
    ...
    if auto_bounds:
        lo_s, hi_s, meta_b = suggest_bounds(raw_hist[pcol], q_low=q_low, q_high=q_high, pad_frac=pad_frac)
    else:
        lo_s, hi_s, meta_b = suggest_bounds(raw_hist[pcol], q_low=0.05, q_high=0.95, pad_frac=0.10)

    default_meta[pcol] = meta_b

    if reset_defaults or (pcol not in st.session_state.bounds_default):
...
# ---- Per Part–Machine override editor ----
...
# Available combos based on model-ready keys (these are what optimization uses)
...
if not parts_k or not machines_k:
    ...
    with colA:
        sel_part_k = st.selectbox("Part (model key)", parts_k, index=0, key="bounds_sel_part_k")
    with colB:
        # machine list filtered for selected part
        ...
        sel_machine_k = st.selectbox("Machine (model key)", machines_for_part, index=0, key="bounds_sel_machine_k")

    group_key = (str(sel_part_k), str(sel_machine_k))

    # NOTE: We do NOT create an override entry unless the user actually changes a bound.

    # Buttons: reset this combo, reset all, copy to all machines for this part
    b1, b2, b3, b4 = st.columns([1, 1, 1, 2])
    with b1:
        reset_this = st.button(
            "Reset this combo",
            help="Remove overrides for this Part–Machine so it uses default bounds.",
    ...
    with b2:
        reset_all = st.button(
            "Reset ALL combos",
            help="Clear every Part–Machine override and return everything to default bounds.",
            use_container_width=True,
    ...
    with b3:
        copy_to_part = st.button(
            ...
            help="Apply this combo's overridden bounds to every machine for the selected part.",
            use_container_width=True,
            key=f"btn_copy_to_part__{group_key[0]}"
        )
    with b4:
        show_all_summary = st.checkbox("Show summary for all combos", value=False, key="chk_show_all_bounds_summary")

    if reset_this:
        # 1) remove stored overrides for this combo
        st.session_state.bounds_overrides.pop(group_key, None)

        # 2) clear widget state for this combo so inputs snap back to defaults
        for pcol in param_cols:
    ...
    if reset_all:
        ...
        # 2) clear *all* override widget keys
    ...
    # Current editable bounds = override if present else defaults
    ...
    for pcol in param_cols:
        ...
        with st.expander(f"{pcol} (default: {def_lo:.6g} … {def_hi:.6g})", expanded=False):
            ...
            if lo >= hi:
                ...
                st.stop()
            # Save ONLY if different from default; otherwise remove override so it doesn't look like an override.
            def_lo, def_hi = st.session_state.bounds_default[pcol]
            # Tolerance to avoid float jitter
            tol = 1e-12 * max(1.0, abs(def_lo), abs(def_hi))
            is_same = (abs(float(lo) - float(def_lo)) <= tol) and (abs(float(hi) - float(def_hi)) <= tol)

            if is_same:
                # Remove this param override if present
                if group_key in st.session_state.bounds_overrides and pcol in st.session_state.bounds_overrides[group_key]:
                    ...
                    # If that was the last override for this combo, remove combo entry entirely
                    if len(st.session_state.bounds_overrides[group_key]) == 0:
                        st.session_state.bounds_overrides.pop(group_key, None)
            else:
                # Create combo override dict if needed, then store param override
    ...
    # If requested, copy these overrides to all machines for this part
    if copy_to_part:
    ...
    # Decide which combos to show
    if show_all_summary:
        combos_to_show = [(r["part_key"], r["machine_key"]) for _, r in combo_df.iterrows()]
    else:
        # Only show combos that actually have at least one override
...
# If auto_run, we also re-run when key inputs change.
# We'll use a simple signature hash stored in session_state.
sig = {
    "part_col": part_col,
    "machine_col": machine_col,

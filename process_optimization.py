...
# BUG FIX: Simple button handling without complex rerun logic
reset_defaults_clicked = st.sidebar.button(
...
if is_dev:
    ...
    with st.expander("Default bounds (apply to ALL Part–Machine combos unless overridden)", expanded=should_expand_bounds):
        ...
        for pcol in param_cols:
            ...
            bounds_edited_this_run = True
        
        # Mark that user has interacted with bounds (keeps expander open)
        if bounds_edited_this_run:
            st.session_state["_bounds_edited"] = True
...
if is_dev:
    ...
    # BUG FIX #1: Initialize machines_for_part safely
...
# -----------------------------------------------------------------------
# BUG FIX #5: Use safe serialization of bounds_overrides in signature hash
# -----------------------------------------------------------------------
sig = {
    "part_col": part_col,
    "machine_col": machine_col,
    ...
    "bounds_overrides": _serialize_bounds_overrides(st.session_state.get("bounds_overrides", {})),

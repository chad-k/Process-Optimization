...
# -----------------------------------------------------------------------
# BUG FIX #2: "Reset ALL default bounds" now also clears the widget keys
# so the number_input widgets visually snap back to the suggested values.
# -----------------------------------------------------------------------
...
for pcol in param_cols:
    ...
    lo_s, hi_s, meta_b = suggest_bounds(
        raw_hist[pcol],
        q_low=q_low if auto_bounds else 0.05,
        q_high=q_high if auto_bounds else 0.95,
        pad_frac=pad_frac if auto_bounds else 0.10,
    )

    default_meta[pcol] = meta_b

    # BUG FIX #2: on reset, delete widget keys so number_inputs re-initialize
    if reset_defaults:
...
if not parts_k or not machines_k:
    ...
    if reset_all:
        ...
        for k in list(st.session_state.keys()):
            if isinstance(k, str) and (k.startswith("ov_min__") or k.startswith("ov_max__")):
                # Parse out the pcol from the key: ov_min__{part}__{machine}__{pcol}
                # Set to default bounds for that param
                parts_key = k.split("__")
                # key format: ov_min__ part __ machine __ pcol (pcol may itself contain __)
                # safer: strip prefix and suffix known portions
                prefix = "ov_min__" if k.startswith("ov_min__") else "ov_max__"
                remainder = k[len(prefix):]  # "{part}__{machine}__{pcol}"
                for pcol in param_cols:
                    suffix = f"__{pcol}"
    ...
    # -----------------------------------------------------------------------
    # Per-parameter override inputs — collected first, THEN copy_to_part runs.
    # BUG FIX #1: moved copy_to_part action to AFTER the loop so it copies
    # the overrides written in this same render pass, not the previous ones.
    # -----------------------------------------------------------------------
    for pcol in param_cols:
        def_lo, def_hi = st.session_state.bounds_default[pcol]
    ...
    # BUG FIX #1: copy_to_part now runs AFTER the loop so it reads
    # the overrides that were just written above.
    if copy_to_part:

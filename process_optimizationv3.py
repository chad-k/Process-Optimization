...
if not parts_k or not machines_k:
    ...
    # Buttons: reset this combo, copy to all machines for this part
    b1, b2, b3 = st.columns([1, 1, 2])
    with b1:
        reset_this = st.button("Reset this combo", help="Remove overrides so this combo uses default bounds.", use_container_width=True)
    with b2:
        copy_to_part = st.button("Copy to all machines (this part)", help="Apply these bounds to every machine for the selected part.", use_container_width=True)
    with b3:
        show_all_summary = st.checkbox("Show summary for all combos", value=False)

    if reset_this:
        st.session_state.bounds_overrides.pop(group_key, None)
        st.success(f"Overrides cleared for {group_key[0]} / {group_key[1]} (now using defaults).")

# -*- coding: utf-8 -*-
"""
Created on Tue Feb 17 20:54:00 2026
...
if not parts_k or not machines_k:
    ...
    with b3:
        copy_to_part = st.button(
            ...
            help="Copy the overrides you've set for the currently selected combo to every other machine under the same part number. Useful when one part runs on multiple machines with the same parameter ranges.",
            use_container_width=True,
    ...
    with b4:
        show_all_summary = st.checkbox(
            ...
            help="When checked, the bounds summary table below shows every Part + Machine combo (including those using default bounds). When unchecked, only combos with active overrides are shown."
        )
        ...
        with st.expander(f"{pcol} (default: {def_lo:.6g} … {def_hi:.6g})", expanded=False):
            ...
            with c1:
                lo = st.number_input(
                    ...
                    help=f"Minimum allowed value for '{pcol}' during optimization for this specific Part + Machine. The optimizer will not suggest a value below this. Default: {def_lo:.6g}"
                )
            with c2:
                hi = st.number_input(
                    f"{pcol} max",
                    ...
                    help=f"Maximum allowed value for '{pcol}' during optimization for this specific Part + Machine. The optimizer will not suggest a value above this. Default: {def_hi:.6g}"
                )
            if lo >= hi:

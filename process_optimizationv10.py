...
def detect_wide_measure_cols(df: pd.DataFrame, patterns: List[str]) -> List[str]:
    ...
    def trailing_num(colname: str) -> Tuple[int, str]:
        """Sort by trailing number first, then by column name (for stable ordering)."""
        m = re.search(r"(\d+)\s*$", str(colname).strip())
        num = int(m.group(1)) if m else 10**9
        return (num, str(colname).lower())
...
def suggest_bounds(
    ...
    if len(s) < 20:
        ...
        # BUG FIX #6: Warn when using fallback (all NaN or very few samples)
        if len(s) == 0:
            src = "fallback(all_nan)"
    else:
...
# BUG FIX #5: Safe serialization of bounds_overrides using base64 encoding
def _encode_group_key(group_key: Tuple[str, str]) -> str:
    """Encode a (part, machine) tuple safely using base64 to avoid delimiter collisions."""
    key_str = json.dumps(list(group_key))
    return base64.b64encode(key_str.encode()).decode()

def _decode_group_key(encoded: str) -> Tuple[str, str]:
    """Decode a base64-encoded group key back to (part, machine) tuple."""
    try:
        key_str = base64.b64decode(encoded.encode()).decode()
        return tuple(json.loads(key_str))
    except Exception:
        return (encoded, "unknown")

# BUG FIX #5: Safe serialization of bounds_overrides for hashing
def _serialize_bounds_overrides(overrides: Dict) -> str:
    """Safely serialize bounds_overrides dict using base64 encoding for keys."""
    safe_dict = {}
    for k, v in overrides.items():
        if isinstance(k, tuple) and len(k) == 2:
            safe_key = _encode_group_key(k)
        else:
            safe_key = str(k)
        safe_dict[safe_key] = v
...
def build_model_df(
    ...
    if used_mode == "Single measurement column":
        ...
        # BUG FIX #10: Track rows with all-NaN subgroup measurements
        all_nan_rows = vals.isna().all(axis=1).sum()
        if all_nan_rows > 0:
            meta["rows_with_all_nan_subgroup"] = int(all_nan_rows)
    ...
    for (part, machine), group in model_df.groupby(["part_key", "machine_key"]):
        ...
        for model_name, model in model_choices.items():
            try:
                ...
                # BUG FIX #5: More informative error messages
                failures.append((part, machine, f"Model {model_name} failed: {str(e)[:50]}"))
                continue
...
5. **Download results**: optimized_machine_settings.csv and optimization_failures.csv

## Notes
- Bounds are auto-suggested from historical data but should always stay within safe operating limits.
- If parameters don't align row-by-row, the app falls back to Part+Machine group averages.
...
with st.sidebar:
    ...
    st.divider()
    st.header("Models")
    st.caption("Using Linear Regression with sensible defaults.")
    ...
    st.caption("Auto-run enabled with standard settings.")
...
# BUG FIX #3: Check for empty model_df after build
if meta['rows_after'] == 0:
    st.error("Model-ready table is empty after building. This usually means:")
    st.markdown("""
    - The Part column in Data doesn't match any parts in Specs
    - All measurements or targets are NaN/invalid
    - All rows were filtered out due to missing required columns
    
    Check your data and column mappings, then reload.
    """)
    st.stop()
...
if meta.get("rows_with_all_nan_subgroup", 0) > 0:
    st.warning(f"⚠️ {meta['rows_with_all_nan_subgroup']} rows had all-NaN subgroup measurements and were filtered out.")
...
# BUG FIX #8: Validate no duplicate parameters
if len(param_cols) != len(set(param_cols)):
    st.error("Duplicate parameter columns selected. Please select each parameter only once.")
    st.stop()
...
# Step 3 — Bounds
# ------------------------------
st.subheader("Step 3 — Bounds (auto-suggested defaults)")

# Fixed bounds settings (Simple mode defaults)
q_low = 0.05
q_high = 0.95
pad_frac = 0.10
...
for pcol in param_cols:
    ...
    if pcol not in st.session_state.bounds_default:
...
# Default bounds editor
with st.expander("Default bounds (apply to all Part–Machine combos)", expanded=False):
    st.caption("Adjust the bounds for each parameter. These bounds are suggested from your historical data but can be manually edited.")
    ...
    for pcol in param_cols:
        ...
        with c2:
            hi = st.number_input(
                ...
                help=f"Maximum bound for '{pcol}'."
            )
        if lo >= hi:
            st.error(f"Invalid bounds for {pcol}: min must be < max.")
            st.stop()
...
# Use safe serialization for hashing
sig = {
    "part_col": part_col,
    "machine_col": machine_col,
    ...
    "bounds_overrides": _serialize_bounds_overrides({}),

...
def compute_results_dynamic(model_df, param_cols, bounds_by_param, soft_tol_percent, model_choices, min_rows_per_group):
    results = []
    failures = []
    ...
    bounds = [(float(bounds_by_param[p][0]), float(bounds_by_param[p][1])) for p in param_cols]
...
# ------------------------------
# Step 3 — Bounds (suggest from RAW data, editable)
# ------------------------------
st.subheader("Step 3 — Bounds (auto-suggested from raw data, editable)")

with st.sidebar:
    st.header("Bounds suggestion")
    auto_bounds = st.checkbox("Auto-suggest bounds", value=True)
    ...
    per_group = st.checkbox("Suggest bounds for one Part+Machine subset", value=False)

raw_hist = data_df.copy()
if per_group:
    parts = sorted(raw_hist[part_col].dropna().astype(str).unique().tolist())
    machines = sorted(raw_hist[machine_col].dropna().astype(str).unique().tolist())
    if parts and machines:
        sel_part = st.selectbox("Subset Part", parts, index=0)
        sel_machine = st.selectbox("Subset Machine", machines, index=0)
        raw_hist = raw_hist[(raw_hist[part_col].astype(str) == str(sel_part)) & (raw_hist[machine_col].astype(str) == str(sel_machine))]
        if raw_hist.empty:
            st.warning("Subset empty; using all rows for bound suggestion.")
            raw_hist = data_df

bounds_by_param: Dict[str, Tuple[float, float]] = {}
reset_all = st.button("Reset all bounds to suggested")

if "bounds_state" not in st.session_state:
    st.session_state.bounds_state = {}

for p in param_cols:
    if p not in data_df.columns:
        st.error(f"Parameter '{p}' not found in DATA CSV.")
        st.stop()

    if auto_bounds:
        lo_s, hi_s, meta_b = suggest_bounds(raw_hist[p], q_low=q_low, q_high=q_high, pad_frac=pad_frac)
    else:
        lo_s, hi_s, meta_b = suggest_bounds(raw_hist[p], q_low=0.05, q_high=0.95, pad_frac=0.10)

    if reset_all or (p not in st.session_state.bounds_state):
        st.session_state.bounds_state[p] = (lo_s, hi_s)

    lo0, hi0 = st.session_state.bounds_state[p]

    with st.expander(f"{p} bounds (suggested: {meta_b['source']} | n={meta_b['n']})", expanded=False):
        cc1, cc2 = st.columns(2)
        with cc1:
            lo = st.number_input(f"{p} min", value=float(lo0), key=f"min_{p}")
        with cc2:
            hi = st.number_input(f"{p} max", value=float(hi0), key=f"max_{p}")

        if lo >= hi:
            st.error(f"Invalid bounds for {p}: min must be < max.")
            st.stop()

        st.session_state.bounds_state[p] = (float(lo), float(hi))
        bounds_by_param[p] = (float(lo), float(hi))
